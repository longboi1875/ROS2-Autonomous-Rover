...
class SafetyMonitor : public rclcpp::Node {
 public:
  SafetyMonitor() : Node("safety_monitor"), logic_(std::chrono::milliseconds(declare_parameter("scan_timeout_ms", 750))) {
    ...
    stop_pub_ = create_publisher<geometry_msgs::msg::Twist>("cmd_vel_safety", 10);
 ...
 private:
  void tick() {
    ...
    if (state != rover_safety::SafetyState::kHealthy) stop_pub_->publish(geometry_msgs::msg::Twist{});
  }
